...
public class PawnMoves implements PieceMovesCalculator {
    ...
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition myPosition) {
        ...
        for (int[] diagonal : diagonals) {
            ...
            ChessPosition diag_pos = new ChessPosition(row, col);
            ChessPiece diag_target = board.getPiece(diag_pos);

            if (diag_target != null && diag_target.getTeamColor() != pawn.getTeamColor()) {
                if (diag_pos.getRow() == promotion_row) {
                    valid_moves.add(new ChessMove(myPosition, diag_pos, ChessPiece.PieceType.BISHOP));
                    valid_moves.add(new ChessMove(myPosition, diag_pos, ChessPiece.PieceType.QUEEN));
                    valid_moves.add(new ChessMove(myPosition, diag_pos, ChessPiece.PieceType.KNIGHT));
                    valid_moves.add(new ChessMove(myPosition, diag_pos, ChessPiece.PieceType.ROOK));
                } else {
                    valid_moves.add(new ChessMove(myPosition, diag_pos, null));
                }
